...
import { BookOpen, Calendar, ShoppingBag, User, Clock, CheckCircle, XCircle } from 'lucide-react';
      ...
      <div className="page-container py-10">
        ...
        <div className="flex items-center gap-4 mb-10 flex-wrap">
          ...
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl font-heading font-bold text-white truncate">Welcome, {currentUser?.name}!</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          {[
            ...
            { icon: User, label: 'Member Since', value: format(new Date(currentUser?.createdAt || Date.now()), 'MMM yyyy'), color: 'text-purple-500' },
            ...
            {profileLoading ? (
              ...
              <div className="space-y-3">
                {activeCourses.slice(0, 5).map((item, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex items-center gap-3 bg-dark-800 border border-dark-700 rounded-xl p-3 hover:border-dark-600 transition-colors">
                      <div className="w-12 h-12 rounded-lg bg-dark-700 overflow-hidden shrink-0">
                        {thumbnail ? (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-dark-400 text-xs mt-0.5 flex items-center gap-1">
                          <Clock size={10} />
                          {item.expiresAt ? (expired ? 'Expired' : `Expires ${format(new Date(item.expiresAt), 'dd MMM yyyy')}`) : 'Lifetime Access'}
                        </p>
                      </div>
                      <Link to={`/courses/${course.slug}`} className="btn-primary btn-sm px-3 py-1.5 text-xs shrink-0">
                        Access
                      </Link>
                    </div>
                  );
            ...
            {ordersLoading ? (
              ...
              <div className="space-y-3">
                {orders.slice(0, 6).map((order) => (
                  <div key={order._id} className="flex items-center gap-3 bg-dark-800 border border-dark-700 rounded-xl p-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                      order.status === 'paid' || order.status === 'free' ? 'bg-green-500/20' : 'bg-red-500/20'
                    }`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-white text-sm truncate">
                        {order.course?.title || order.event?.title || order.package?.title || 'Order'}
                      </p>
                      <p className="text-dark-400 text-xs capitalize">{order.orderType} • {order.status}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-white text-sm font-medium">
                        {order.isFree || order.amount === 0 ? 'FREE' : `₹${(order.amount / 100).toLocaleString()}`}
                      </p>
                      <p className="text-dark-500 text-xs">{format(new Date(order.createdAt), 'dd MMM')}</p>
                    </div>
                  </div>
                ))}
              </div>
